    ...
    column_map = frappe._dict(
        {
            ...
            "qty_uom": {"label": f"Qty ({filters.get('uom')})", "fieldname": "qty_uom", "fieldtype": "Float", "width": 80} if filters.get("uom") else {"label": f"Qty ({filters.get('uom')})", "fieldname": "qty_uom", "fieldtype": "Float", "width": 80,"hidden":1},
...
class GrossProfitGenerator(object):
                    ...
                    if row.indent == 1.0:
                        if (
                            ...
                            for returned_item_row in returned_item_rows:
                                ...
                                row.base_amount += flt(returned_item_row.base_amount,
                                                       self.currency_precision)
                            row.buying_amount = flt(
                ...
                for i, row in enumerate(self.grouped[key]):
                    if i == 0:
                        ...
                        if self.filters.get("group_by") == "Sales Person":
                            new_row.allocated_amount += flt(row.allocated_amount,
                                                            self.currency_precision)
                new_row = self.set_average_rate(new_row)
                self.grouped_data.append(new_row)
    ...
    def calculate_buying_amount_from_sle(self, row, my_sle, parenttype, parent, item_row, item_code):
        for i, sle in enumerate(my_sle):
            ...
            if (
                ...
                previous_stock_value = len(my_sle) > i + \
                    1 and flt(my_sle[i + 1].stock_value) or 0.0
    ...
    def group_items_by_invoice(self):
        ...
        for row in self.si_list:
            ...
            grouped.setdefault(row.parent, [self.get_invoice_row(row)]).append(
                row.update(
                    {"indent": 1.0, "parent_invoice": row.parent,
    ...
    def calculate_qty_in_chosen_uom(item_code, qty, chosen_uom, sales_invoice_uom, default_uom):
        if chosen_uom != default_uom:
            conversion_factor = GrossProfitGenerator.get_conversion_factor(
