...
def get_columns(salary_slips):
	...
	salary_component_query = (
		frappe.qb.from_(salary_detail_doc)
	...
	conditions = [salary_detail_doc.parent.like("%" + name + "%") for name in salary_slips]
	...
	if conditions:
		combined_condition = (
			conditions[0]
			if len(conditions) == 1
			else reduce(lambda x, y: x | y, conditions[1:], conditions[0])
		)
