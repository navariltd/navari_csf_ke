...
def get_p10_report_data(filters):
	...
	if filters.get("from_date") and filters.get("to_date"):
		conditions.append(salary_slip.posting_date.between(filters.get("from_date"), filters.get("to_date")))
	...
	query = (
		...
		.select(
			...
			Case().when(salary_detail.amount.isnull(), 0).else_(salary_detail.amount).as_("amount"),
		)
		.where(
