...
def execute(filters=None):
    ...
    data = group_per_deparment(filters)
    print("DATA", data)
...
def get_columns(filters, prev_month, prev_year):
    cur_month_name = calendar.month_name[getdate(filters.to_date).month]
...
def get_basic_data(data, salary_slips, currency, company_currency, prev_ss_basic):
    ss_basic_map = get_ss_basic_map(
        salary_slips
    )  # These are salary slips for the current month (I call it but should be previous)

    total_cur_basic = 0
    unique_cur_basic_salary_components = []  # Unique current basic salary components
    unique_prev_basic_salary_components = (
        []
    )  # Unique current previous salary components
    ...
    # My hook should be somewhere here
...
def get_prev_month_date(filters):
    prev_month = getdate(filters.from_date).month
...
def get_prev_salary_slips(filters, company_currency, prev_first_date, prev_last_date):
    filters = {
        ...
        "to_date": "2024-11-30",
        "currency": "KES",
        "company": "Cecypo International Limited",
        "docstatus": "Submitted",
    }
...
def get_salary_slips2(filters, company_currency):
    ...
    print("SALARY SLIPS", salary_slips)
    return salary_slips


def get_conditions2(filters, company_currency):
    conditions = ""
    doc_status = {"Draft": 0, "Submitted": 1, "Cancelled": 2}
    ...
    if filters.get("start_date"):
        conditions += " and start_date >= %(start_date)s"
...
def group_per_deparment(filters):
    company_currency = "KES"
    first_date = ""
    last_date = ""
    old_ss_filters = filters.copy()
    old_ss_filters.update({"start_date": "01-04-2025", "end_date": "30-04-2025"})

    new_ss_filters = filters.copy()
    new_ss_filters.update({"start_date": "01-05-2025", "end_date": "30-05-2025"})

    print("FILTERS", new_ss_filters)

    # Get previous month - 1 salary slips
    old_salary_slips = get_salary_slips2(old_ss_filters, company_currency)

    # Get previous month salary slips
    new_salary_slips = get_salary_slips2(new_ss_filters, company_currency)

    if old_salary_slips and new_salary_slips:
        data = get_earnings_data2(old_salary_slips, new_salary_slips)
