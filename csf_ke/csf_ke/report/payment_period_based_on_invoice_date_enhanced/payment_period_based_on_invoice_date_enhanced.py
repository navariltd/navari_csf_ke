...
def execute(filters=None):
    ...
    invoice_details = get_salesperson_and_invoice_posting_date_map(filters)
    ...
    for d in entries:
        ...
        if filters.get("payment_type") == _("Incoming"):
            row.insert(6, invoice.sales_person)
...
def validate_filters(filters):
    if (
        filters.get("payment_type") == _("Incoming")
        and filters.get("party_type") == "Supplier"
    ) or (
        filters.get("payment_type") == _("Outgoing")
        and filters.get("party_type") == "Customer"
    ):
        frappe.throw(
...
def get_salesperson_and_invoice_posting_date_map(filters):
    invoice_details = {}
    dt = (
    ...
    # If payment type is incoming, join with sales team table
    if filters.get("payment_type") == _("Incoming"):
        sales_team = qb.DocType("Sales Team")
        query = (
            query.left_join(sales_team)
            .on(sales_team.parent == dt.name)
