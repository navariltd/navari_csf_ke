...
from frappe.query_builder.functions import Coalesce, Timestamp
from frappe.utils import add_days, cint, date_diff, flt, get_datetime, get_time, getdate
...
class StockBalanceReport:
	def __init__(self, filters: StockBalanceFilter | None) -> None:
		...
		from_time = get_time(filters.get("from_time") or "00:00:00")
		to_time = get_time(filters.get("to_time") or "23:59:59")

		self.from_datetime = get_datetime(f"{self.from_date} {from_time}")
		self.to_datetime = get_datetime(f"{self.to_date} {to_time}")
	...
	def prepare_item_warehouse_map(self, item_warehouse_map, entry, group_by_key):
		...
		entry_datetime = get_datetime(f"{entry.posting_date} {entry.get('posting_time', '00:00:00')}")

		if entry_datetime < self.from_datetime or entry.voucher_no in self.opening_vouchers.get(
			entry.voucher_type, []
		):
			qty_dict.opening_qty += qty_diff
			qty_dict.opening_val += value_diff

		elif entry_datetime >= self.from_datetime and entry_datetime <= self.to_datetime:
			if flt(qty_diff, self.float_precision) >= 0:
				qty_dict.in_qty += qty_diff

				if entry.voucher_type in ["Purchase Invoice", "Purchase Receipt"]:
					qty_dict.purchased_qty += qty_diff
				if entry.voucher_type == "Stock Reconciliation":
					qty_dict.stock_recon_in += qty_diff
				if entry.voucher_type == "Stock Entry":
					if entry.stock_entry_type in ["Material Receipt", "Material Transfer"]:
						qty_dict.stock_in_qty += qty_diff
					elif entry.purpose == "Manufacture":
						qty_dict.manufactured_qty += qty_diff

			else:
				qty_dict.out_qty += abs(qty_diff)

				if entry.voucher_type == "Stock Entry":
					if entry.stock_entry_type in ["Material Transfer", "Material Issue"]:
						qty_dict.stock_out_qty += abs(qty_diff)

					elif entry.stock_entry_type == "Manufacture":
						qty_dict.consumed_qty += abs(qty_diff)

				elif entry.voucher_type in ["Sales Invoice", "Delivery Note"]:
					qty_dict.sales_qty += abs(qty_diff)

				if entry.voucher_type == "Stock Reconciliation":
					qty_dict.stock_recon_out += abs(qty_diff)
	...
	def prepare_stock_ledger_entries(self):
		...
		# Change method name to use datetime filters instead
		query = self.apply_datetime_filters(query, sle)
	...
	def apply_datetime_filters(self, query, sle) -> str:
		"""Apply date and time filters to the query."""
		if not self.filters.ignore_closing_balance and self.start_from:
			start_datetime = get_datetime(f"{self.start_from} 00:00:00")
			query = query.where(sle.posting_datetime >= start_datetime)
	...
	def get_columns(self):
		...
		columns.extend(
			[
				...
				{
					"label": _("Stock Recon (In)"),
					"fieldname": "stock_recon_in",
					"fieldtype": "Float",
					"width": 100,
					"convertible": "qty",
				},
				{
					"label": _("Total Out Qty"),
					"fieldname": "out_qty",
					"fieldtype": "Float",
					"width": 80,
					"convertible": "qty",
				...
				{
					"label": _("Stock Recon (Out)"),
					"fieldname": "stock_recon_out",
					"fieldtype": "Float",
					"width": 100,
					"convertible": "qty",
				},
	...
	def get_opening_vouchers(self):
		...
		vouchers_data = (
			frappe.qb.from_(
				(
					...
					.where(
						(se.docstatus == 1)
						& (Timestamp(se.posting_date, se.posting_time) <= self.to_datetime)
						& (se.is_opening == "Yes")
					)
				)
				+ (
					...
					.where(
						(sr.docstatus == 1)
						& (Timestamp(sr.posting_date, sr.posting_time) <= self.to_datetime)
		...
		if vouchers_data:
			for d in vouchers_data:
				if d["voucher_type"] in opening_vouchers:
