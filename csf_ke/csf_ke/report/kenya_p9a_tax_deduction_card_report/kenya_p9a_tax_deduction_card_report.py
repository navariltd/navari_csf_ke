...
def execute(filters=None):
    ...
    if not fiscal_year:
        ...
        p9a_tdc_year = p9a_tdc_year_name[
            0:4
        ]  # for cases where fiscal year starts between the year
    ...
    for emp in employees:
        for month_idx in range(1, no_of_months):
            month_date_string = (
                cstr(p9a_tdc_year) + "-" + cstr(month_idx) + "-" + cstr(1)
            )
            ...
            e1_defined_contribution_retirement_scheme_amt = flt(
                (30 / 100) * basic_salary_amt
            )

            e2_defined_contribution_retirement_scheme_amt = (
                get_p9a_tax_deduction_card_amt(
                    filters,
                    emp.name,
                    month_start_date,
                    month_end_date,
            ...
            if basic_salary_amt > 0:
                e3_defined_contribution_retirement_scheme_amt = (
                    get_p9a_tax_deduction_card_fixed_component_amt(
...
def get_months(period_start_date, period_end_date):
    ...
    diff = (12 * end_date.year + end_date.month) - (
...
def get_p9a_tax_deduction_card_amt(
    ...
    salary_slip_query = (
        ...
        .select(
            ...
            (
                salary_detail_doc.amount.as_("amt") if salary_detail_doc.amount else 0
            ).as_("amt"),
    ...
    for d in ss_p9a_tax_deduction_card_component_:
        if currency != company_currency:
            p9a_tax_deduction_card_amount += flt(d.amt) * flt(
...
def get_p9a_tax_deduction_card_gross_pay(
    ...
    salary_slip_query = (
        ...
        .select(
            ...
            salary_slip_doc.company,
            (
                salary_slip_doc.gross_pay.as_("amt") if salary_slip_doc.gross_pay else 0
            ).as_("amt"),
            salary_slip_doc.exchange_rate,
        )
        .where(
    ...
    for d in ss_p9a_tax_deduction_card_gross_pay_:
        if currency != company_currency:
            p9a_tax_deduction_card_gross_pay += flt(d.amt) * flt(
                d.exchange_rate if d.exchange_rate else 1
            )
        else:
...
def get_p9a_tax_deduction_card_fixed_component_amt(p9a_tax_deduction_card_type):
    ...
    salary_component_query = (
        ...
        .select(
            (
                salary_component_doc.amount.as_("amt")
                if salary_component_doc.amount
                else 0
            ).as_("amt")
        )
        .where(
