...
class Analytics:
	...
	def get_sales_transactions_based_on_order_type(self):
		...
		self.entries = frappe.db.sql(
			f""" select s.order_type as entity, s.{value_field} as value_field, s.{self.date_field}
            from `tab{self.filters.doc_type}` s where s.docstatus = 1 and s.company = %s and s.{self.date_field} between %s and %s
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = frappe.db.sql(
			f"""
            select i.item_code as entity, i.item_name as entity_name, i.stock_uom, i.{value_field} as value_field, s.{self.date_field}
