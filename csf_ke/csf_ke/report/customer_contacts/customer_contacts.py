...
def execute(filters=None):
	...
	emails = frappe.get_all(
		"Contact Email", filters={"parent": ["in", contact_ids]}, fields=["parent", "email_id"]
	)
	phones = frappe.get_all(
		"Contact Phone", filters={"parent": ["in", contact_ids]}, fields=["parent", "phone"]
	)
	...
	for account_manager, customers in account_manager_map.items():
		...
		for customer_name, contacts in customers.items():
			...
			final_data.append(
				{
					"Account Manager": f'<a href="/app/user/{account_manager_id}">{account_manager}</a>'
					if customer_name == first_customer
					else "",
