...
def get_data(filters, company_currency):
	...
	old_ss_filters.update({"start_date": filters.get("from_date"), "end_date": old_end_date})

	new_ss_filters = filters.copy()
	new_ss_filters.update({"start_date": new_start_date, "end_date": filters.get("to_date")})
	...
	deductions_data = get_salary_slip_data(old_salary_slips, new_salary_slips, "deductions")
	...
	if earnings_data and deductions_data:
		if filters.get("based_on") == "Department":
			...
			grouped_data = get_comparison_per_employee(filters, earnings_data, deductions_data, loans)
		else:
	...
	columns = [
		{
			...
			"label": _(f"{old_month_name} {old_date.year}"),
			"fieldtype": "Float",
			"width": 150,
			"precision": 2,
		},
		{
			"fieldname": "total",
			"label": _(f"{new_month_name} {new_date.year}"),
			"fieldtype": "Float",
			"width": 200,
			"precision": 2,
		},
...
def get_comparison_per_company(filters, earnings_data, deductions_data, old_ss_count, new_ss_count, loans):
	if loans:
	...
	for company in sorted(grouped.keys()):
		...
		if deductions and not filters.get("component_type") == "Earnings":
			...
			if loans:
				...
				final_output.append(
					{
						"company": (company if filters.get("component_type") == "Deductions" else None),
				...
				final_output.append(
					{
						...
						"total_prev_month": loan.get("total_prev_month", 0),
						"total": loan.get("total", 0),
						"difference_amount": loan.get("total", 0) - loan.get("total_prev_month", 0),
					}
				)
	...
	for emp in sorted(grouped.keys()):
		...
		if earnings and not filters.get("component_type") == "Deductions":
			...
			for earning in earnings:
				...
				new_earning["difference_amount"] = new_earning.get("total", 0) - new_earning.get(
					"total_prev_month", 0
				)
				final_output.append(new_earning)
		...
		if deductions and not filters.get("component_type") == "Earnings":
			...
			for deduction in deductions:
				...
				new_deduction["difference_amount"] = new_deduction.get("total", 0) - new_deduction.get(
					"total_prev_month", 0
				)
