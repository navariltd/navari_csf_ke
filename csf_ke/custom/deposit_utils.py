...
def validate_invoice(doc, methods):
	...
	order_doctype, order_field, _partner = get_invoice_order_type(doc.doctype)
...
def cancel_deposit_invoice(doc, method):
	...
	if existing_invoices:
		link = get_link_to_form(order_doctype, linked_doc)
		frappe.throw(
			f"Unable to cancel the deposit invoice for order {link}.<br/>Please cancel the normal invoice first."
		)
...
def apply_deposit_deduction(doc, method):
	...
	doc.items = [
		item for item in doc.items if not frappe.db.get_value("Item", item.item_code, "is_deposit_item")
	]
...
def create_deposit_invoice(
	...
	deposit_account_field = (
		"sales_deposit_account" if order_doctype == "Sales Order" else "purchase_deposit_account"
	)
	...
	def set_missing_values(source, target):
		...
		deposit_item = frappe.call("csf_ke.custom.item.get_deposit_item", company=source.company)
		if not deposit_item:
